...
class ProducerTest extends JUnitSuite {
  ...
  def testZKSendToNewTopic() {
    ...
    try {
      ...
      producer.send(new ProducerData[String, String]("new-topic", "test", asJavaList(Array("test1"))))
      Thread.sleep(100)
      producer.send(new ProducerData[String, String]("new-topic", "test", asJavaList(Array("test1"))))
      Thread.sleep(100)
  ...
  def testPartitionedSendToNewTopic() {
    ...
    syncProducer1.send("test-topic1", 0, new ByteBufferMessageSet(compressionCodec = NoCompressionCodec,
                                                                  messages = asJavaList(Array(new Message("test1".getBytes)))))
    EasyMock.expectLastCall
    syncProducer1.send("test-topic1", 0, new ByteBufferMessageSet(compressionCodec = NoCompressionCodec,
                                                                  messages = asJavaList(Array(new Message("test1".getBytes)))))
    EasyMock.expectLastCall
    ...
    producer.send(new ProducerData[String, String]("test-topic1", "test", asJavaList(Array("test1"))))
    Thread.sleep(100)
    ...
    producer1.send("test-topic1", new ByteBufferMessageSet(compressionCodec = NoCompressionCodec,
                                                           messages = asJavaList(Array(new Message("test".getBytes())))))
    Thread.sleep(100)

    // wait for zookeeper to register the new topic
    producer.send(new ProducerData[String, String]("test-topic1", "test1", asJavaList(Array("test1"))))
    Thread.sleep(100)
    producer.close
